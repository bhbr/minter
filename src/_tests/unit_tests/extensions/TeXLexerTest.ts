...
import { TeXLexer } from 'extensions/creations/VisualAlgebra/model/TeXLexer'
	...
	subtests: [
		new ValueTest({
			...
			function: function(): Array<string> {
				return TeXLexer.texToSentence('1.23')
			},
			value: ['1.23']
		}),
		new ErrorTest({
			...
			function: function(): Array<string> {
				return TeXLexer.texToSentence('1.2.3')
			}
		}),
		new ValueTest({
			name: 'Additions of numbers (without whitespace) get lexed correctly',
			function: function(): Array<string> {
				return TeXLexer.texToSentence('1+2')
			},
			value: ['1', '+', '2']
		}),
		new ValueTest({
		...
		new ValueTest({
			...
			function: function(): Array<string> {
				return TeXLexer.texToSentence('\\sin\\sqrt{1+2}')
			},
			value: ['\\sin', '\\sqrt', '{', '1', '+', '2', '}']
		}),
		new ValueTest({
		...
		new ValueTest({
			...
			function: function(): Array<string> {
				return TeXLexer.texToSentence('\\frac {\\sqrt[3] {5}} {\\log_2 ( x + 1 ) }')
			},
