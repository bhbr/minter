...
export class IntersectionPoint extends Point {
	...
	arrowCircleIntersection(arrow: Arrow, circle: Circle, index: number): Vertex {
		...
		if (d >= 0) {
			let l: number = (-b + (index == 0 ? -1 : 1) * d**0.5)/(2*a)
	...
	arrowArrowIntersection(arrow1: Arrow, arrow2: Arrow) {
		...
		let lambda: number = (CD.y*AC.x - CD.x*AC.y)/det
		let mu: number = (AB.y*AC.x - AB.x*AC.y)/det
		let Q: Vertex = A.add(AB.multiply(lambda))

		let intersectionFlag1: boolean = (arrow1.constructor.name == 'Segment' && lambda > 0 && lambda < 1) || (arrow1.constructor.name == 'Ray' && lambda > 0) || (arrow1.constructor.name == 'Line')
		let intersectionFlag2: boolean = (arrow2.constructor.name == 'Segment' && mu > 0 && mu < 1) || (arrow2.constructor.name == 'Ray' && mu > 0) || (arrow2.constructor.name == 'Line')
