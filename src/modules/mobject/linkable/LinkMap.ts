...
import { ScreenEvent, ScreenEventHandler, isTouchDevice } from '../screen_events'
import { Dependency } from '../Dependency'
import { log, remove, extend } from '../../helpers/helpers'
...
export class LinkMap extends Mobject {
	...
	hookAtLocation(p: Vertex): LinkHook | null {
		for (let h of (this.parent as ExpandableMobject).innerInputHooks()) {
			if (p.closeTo(h.positionInLinkMap(), SNAPPING_DISTANCE)) {
				return h
			}
		}
		for (let h of (this.parent as ExpandableMobject).innerOutputHooks()) {
			if (p.closeTo(h.positionInLinkMap(), SNAPPING_DISTANCE)) {
				return h
			}
		}
	...
	connectedMobjectOfOpenLink(): LinkableMobject | null {
		let h = this.connectedHookOfOpenLink()
		if (h === null) { return null }
		return h.parent.parent as LinkableMobject
	}
